...
6. Pick all table mugs (3) -> box-inside
"""
import os
import sys
import json
...
def _select_runtime_box_grocery(env, exclude_names=None):
    ...
    for name in discovered:
        ...
        if (h is not None) and (h in excluded):
            continue
        in_box_region = _is_in_region(env, obj, "box-inside")
...
def main():
    ...
    if missing:
        ...
        summary = _build_gt_summary([], 0.0, failure_reason=f"missing_objects:{missing}")
