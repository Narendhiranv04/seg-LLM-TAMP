...
class Variation3HardLiveSegRunner:
    ...
    def _on_primitive_action(self, action_name, action_label):
        if self.viewer is None or self.current_action_index < 0:
            return
        primitive_label = (action_label or action_name or "action").strip()
        base_label = self.action_sequence[self.current_action_index]
        try:
            self.viewer.set_current_action(
                action_index=self.current_action_index,
                action_label=f"{base_label} [{primitive_label}]",
            )
        except Exception:
            pass
    ...
    def run(self):
        ...
        v3.base.ACTION_PROGRESS_CALLBACK = self._on_primitive_action
