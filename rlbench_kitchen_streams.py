...
def fn_sample_place_kin(o, p, r):
    ...
    try:
        print(f"DEBUG [Stream]: Calling compute_place_trajectory for {o} at {p} in region {r}")
        grasp, q1, q2, traj_tuple = ENV.compute_place_trajectory(obj, list(p), region_name=r)
        print(f"DEBUG [Stream]: Success! Found base place trajectory for {o}")
        
        # Enforce "Retreat to Home" constraint
        print(f"DEBUG [Stream]: Planning retreat to home from {q2}")
        q_home, traj_home = ENV.compute_retreat_to_home(q2)
        
        if traj_home:
            print(f"DEBUG [Stream]: Success! Retreat to home found.")
            ...
            print(f"DEBUG [Stream]: Failed to plan retreat to home from {q2}")
    except Exception as e:
        print(f"DEBUG [Stream]: compute_place_trajectory failed for {o}: {e}")
        import traceback
        traceback.print_exc()
        return
...
def fn_sample_motion(q1, q2):
    ...
    print(f"DEBUG [Stream]: Calling compute_motion_plan from {q1} to {q2}")
    path = ENV.compute_motion_plan(q1, q2) 
    if path is None:
        print(f"DEBUG [Stream]: compute_motion_plan FAILED")
        return None
    print(f"DEBUG [Stream]: Success! Found motion path (len={len(path)})")
    yield (path,)
