...
from vlm_pipeline.model_registry import ModelSpec
...
class RemoteVLMPlanner:
    ...
    def __init__(self, server_url: str = None, use_vision: bool = True, request_timeout_s: float = None, expected_model: Optional[ModelSpec] = None):
        """
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[RemoteVLM] Model alias: {data.get('model_alias', '(unknown)')}")
                print(f"[RemoteVLM] Model path : {data.get('model_name')}")
                print(f"[RemoteVLM] Model type : {data.get('model_type', '(unknown)')}")
                print(f"[RemoteVLM] GPU available: {data.get('gpu_available')}")
                if self.expected_model is not None:
                    server_type = data.get('model_type')
                    server_name = data.get('model_name')
                    if server_type and server_type != self.expected_model.model_type:
                        print(
                            f"[RemoteVLM] WARNING: expected model type {self.expected_model.model_type} "
                            f"but server reports {server_type}"
                        )
                    if server_name and server_name != self.expected_model.path:
                        print(
                            f"[RemoteVLM] WARNING: expected model path {self.expected_model.path} "
                            f"but server reports {server_name}"
                        )
                return self.loaded
            else:
    ...
    def get_server_debug_info(self) -> Dict[str, Any]:
        """Fetch health and last-request debug information from the remote server."""
        info: Dict[str, Any] = {"health": {}, "last_request": {}}
        try:
