...
# Geometric utilities
from .geometric_utils import resolve_region, is_inside_xy
...
class VLMContextAggregator:
    ...
    def __init__(self,
                 ...
                 plan_prompt_path: Optional[str] = None,
                 lean: bool = False):
        """
        ...
        Args:
            ...
            plan_prompt_path: Path to custom initial plan user prompt file
            lean: If True, use token-efficient 'Lean' state representation
        """
        self.env = env
        self.lean = lean
        ...
        if system_prompt_path and os.path.exists(system_prompt_path):
            try:
                ...
                print(f"[Context Aggregator] Loaded system prompt from {system_prompt_path}")
            except Exception as e:
                print(f"[Context Aggregator] Error loading system prompt: {e}")

        self.plan_prompt_template = None
        if plan_prompt_path and os.path.exists(plan_prompt_path):
            try:
                with open(plan_prompt_path, 'r') as f:
                    self.plan_prompt_template = f.read()
                print(f"[Context Aggregator] Loaded plan prompt from {plan_prompt_path}")
            except Exception as e:
                print(f"[Context Aggregator] Error loading plan prompt: {e}")
        ...
        # Define regions with their simulator shape names
        self.regions = [
            {'name': 'table', 'scene_name': 'diningTable', 'description': 'main dining table surface'},
            {'name': 'box-top', 'scene_name': 'box_lid', 'description': 'top surface of the closed box'},
            {'name': 'box-inside', 'scene_name': 'box_boundary', 'description': 'interior of the box'},
            {'name': 'placement_boundary', 'scene_name': 'placement_boundary', 'description': 'target area for placing mugs'},
            {'name': 'cupboard_boundary', 'scene_name': 'cupboard_boundary', 'description': 'inside the cupboard shelf'},
            {'name': 'cupboard_boundary_top', 'scene_name': 'cupboard_boundary_top', 'description': 'top cupboard shelf'},
            {'name': 'groceries_boundary', 'scene_name': 'groceries_boundary', 'description': 'groceries storage area'},
        ]
    ...
    def update_visible_objects(self, event: str = "") -> Dict[str, List[str]]:
        ...
        try:
            ...
            # --- PURU_LOG SEGMENTATION ---
            try:
                import os, time
                log_dir = "puru_log"
                os.makedirs(log_dir, exist_ok=True)
                timestamp = time.strftime("%Y%m%d_%H%M%S")
                seg_log_path = os.path.join(log_dir, "segmentation_history.txt")
                with open(seg_log_path, "a", encoding="utf-8") as f:
                    f.write(f"[{timestamp}] Event '{event}': raw segmentation output = {sorted(current_scene)}\n")
            except Exception as e:
                pass
            # -------------------------------
    ...
    def get_scene_state(self) -> SceneState:
        """
        Extract current scene state from the environment using geometric resolution.
        """
        objects = []
        missing = []

        # 1. Update visible objects from segmentation
        self._initialize_visibility_tools()
        visible_pddl = set()
        if self.segmentation_detector:
            snapshot = self.update_visible_objects(event="scene-state")
            visible_pddl = set(snapshot.get("visible_pddl", []))
            
        # If not in visible-only mode, we consider all expected objects
        process_list = visible_pddl if self.visible_objects_only else set(self.expected_objects.keys())
        missing = sorted(set(self.expected_objects.keys()) - visible_pddl)

        # 2. Extract Region Bounding Boxes from Simulator (Map Ground Truth)
        region_map = {}
        if self.segmentation_detector:
            for r in self.regions:
                bb = self.segmentation_detector.get_bounding_box(r['scene_name'])
                if bb:
                    region_map[r['name']] = bb

        # 3. Check Lid State (Physics check)
        lid_state = "closed"
        if self.env:
            try:
                ...
                if lid_obj and box_base:
                    ...
                    # Offset check: if lid is moved off center, it's open
                    x_offset = abs(lid_pos[0] - box_pos[0])
                    lid_state = "open" if x_offset > 0.10 else "closed"
            except: pass

        # 4. Resolve every object geometrically
        for pddl_name in process_list:
            info = self.expected_objects.get(pddl_name)
            if not info or pddl_name == 'box_lid': continue
            
            # Get object world coordinate
            # Use segmentation detector if available, otherwise fall back to env directly
            pos = None
            if self.segmentation_detector is not None:
                pos = self.segmentation_detector.get_object_pose(pddl_name)
            elif self.env is not None:
                obj = self.env.get_object(pddl_name)
                if obj is not None:
                    raw = obj.get_position()
                    pos = tuple(raw)  # convert numpy array to tuple
            if pos is None: continue
            
            # Resolve region and location string
            region, location = resolve_region(pos, region_map)
            
            # Override for special cases (e.g., lid blocking)
            is_pickable = True
            blocked_by = None
            
            if region == 'box-inside':
            ...
            objects.append(ObjectState(
                ...
                visible_in_frames=(pddl_name in visible_pddl)
            ))

        # Add the lid itself
        if 'box_lid' in process_list or not self.visible_objects_only:
            objects.append(ObjectState(
                name='box_lid',
                pddl_name='box_lid',
                obj_type='lid',
        ...
        # 5. Inter-object blocking (e.g., mug on lid blocks lid)
        # Check if any object's X-Y is within box-top region
        objects_on_lid = [o.pddl_name for o in objects if o.region == 'box-top']
        if objects_on_lid:
            for o in objects:
                if o.name == 'box_lid':
                    o.blocked_by = objects_on_lid[0]

        visibility_mode = "visible-only" if self.visible_objects_only else "full-scene"
        return SceneState(
            objects=objects,
            regions=[{'name': r['name'], 'description': r['description']} for r in self.regions],
            robot_gripper_state="empty",
            robot_holding=None, # TBD: Add gripper check if needed
            lid_state=lid_state,
            expected_objects=list(self.expected_objects.keys()),
            missing_objects=missing,
            visibility_mode=visibility_mode,
            visible_objects=sorted(list(visible_pddl)) if visibility_mode == "visible-only" else sorted([o.pddl_name for o in objects]),
...
Do NOT include any explanation, just the action sequence."""
    ...
    def _get_lean_state_text(self, state: SceneState) -> str:
        """Helper to get lean state text (used for prompt and preview)."""
        state_lines = []
        state_lines.append(f"Robot: hand-empty={state.robot_gripper_state == 'empty'}")
        if state.robot_holding:
            state_lines.append(f"Robot: holding={state.robot_holding}")
        
        for obj in state.objects:
            blocked = f", blocked-by={obj.blocked_by}" if obj.blocked_by else ""
            state_str = f", state={obj.state}" if obj.state else ""
            state_lines.append(f"- {obj.pddl_name}: at={obj.region}, loc={obj.location}{state_str}{blocked}")
    ...
    def build_user_prompt(self, state: SceneState, goal: str, 
        ...
        """
        # --- LEAN STATE MODE ---
        if self.lean and not is_replan:
            state_text = self._get_lean_state_text(state)
            
            if self.plan_prompt_template:
                try:
                    ...
                    print(f"[Context Aggregator] Prompt formatting error: {e}")
            
            return f"### SCENE STATE\n{state_text}\n\n### GOAL\n{goal}"

        # --- STANDARD VERBOSE MODE (or replan) ---
...
The attached image shows the current scene.
...
Please provide a NEW action sequence that avoids the failure."""
        else:
            # Plan prompt template injection
            if self.plan_prompt_template:
                try:
                    return self.plan_prompt_template.format(
                        STATE_TEXT=state_text,
...
Based on the visual scene and state description, provide the action sequence to achieve the goal."""
    ...
    def create_prompt_bundle(self, goal: str,
        ...
        # breakpoint() # [DEBUG 1]: After the initial state is generated
