...
class ReplanConfig:
    ...
    # Custom Prompts (Phase 1)
    system_prompt_path: Optional[str] = None
    plan_prompt_path: Optional[str] = None
    replan_prompt_path: Optional[str] = None
    lean: bool = False
...
class VLMReplanningPipeline:
    ...
    def __init__(self, config: ReplanConfig = None):
        ...
        self.context_aggregator = VLMContextAggregator(
            ...
            plan_prompt_path=self.config.plan_prompt_path,
            lean=self.config.lean
        )
    ...
    def build_replan_prompt(self, 
        ...
        """
        # Load custom replan prompt if specified
        if self.config.replan_prompt_path and os.path.exists(self.config.replan_prompt_path):
            try:
                with open(self.config.replan_prompt_path, 'r') as f:
                    template = f.read()
                
                # Assemble context for formatting
                ...
                failed_str = f"  ❌ {failed_action}" if failed_action is not None else "  (scene change interruption)"
                
                return template.format(
                    GOAL_TEXT=original_goal,
                    COMPLETED_ACTIONS=completed_str,
                    FAILED_ACTION=failed_str,
                    ERROR_MESSAGE=error_message,
                    REMAINING_ACTIONS=remaining_str,
                    STATE_TEXT=state_text
                )
            except Exception as e:
                print(f"[ReplanPipeline] Error formatting custom replan prompt: {e}")
...
..."""
    ...
    def run(self, goal: str) -> Dict[str, Any]:
        ...
        while replan_count <= self.config.max_replans:
            ...
            # breakpoint() # [DEBUG 7]: After any action fails to execute
...
def main():
    ...
    parser.add_argument("--plan-prompt", type=str, help="Path to custom plan user prompt file")
    parser.add_argument("--replan-prompt", type=str, help="Path to custom replan prompt file")
    parser.add_argument("--lean", action="store_true", help="Use token-efficient lean state representation")
    ...
    config = ReplanConfig(
        ...
        plan_prompt_path=args.plan_prompt,
        replan_prompt_path=args.replan_prompt,
        lean=args.lean
    )
