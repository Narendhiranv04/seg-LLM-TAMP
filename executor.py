...
class DirectPrimitiveExecutor(VLMExecutorV2):
    ...
    def _dispatch_action(self, action: DirectAction, next_action: Optional[DirectAction] = None) -> Tuple[bool, str]:
        ...
        if action.action_name == 'pick':
            ...
            if object_name == 'mug3':
                self._cupboard_picked = True
                return self._execute_cupboard_pick(object_name)
            self._cupboard_picked = False
            return self._execute_pick_pddl(object_name)
        if action.action_name == 'place':
            object_name, target_region = action.args
            # Only use cupboard-specific scripted place if the object was PICKED FROM
            # the cupboard (horizontal approach). For placing INTO the cupboard from
            # the table, use the standard PDDL solver (matching GT behavior).
            if target_region in ('cupboard_boundary', 'cupboard_boundary_top') and getattr(self, '_cupboard_picked', False):
                self._cupboard_picked = False
    ...
    def _execute_move_token(self, next_action: Optional[DirectAction] = None) -> Tuple[bool, str]:
        ...
        if next_action is not None and next_action.action_name == 'place':
            ...
            # Only use cupboard hover transit if the object was picked from the cupboard
            if target_region in ('cupboard_boundary', 'cupboard_boundary_top') and getattr(self, '_cupboard_picked', False):
                return self._move_to_cupboard_place_hover(object_name, target_region)
            # Standard PDDL pre-solve for all other cases (including placing INTO cupboard from table)
    ...
    def _presolve_place(self, object_name: str, target_region: str) -> Optional[bool]:
        ...
        # Route through home position first to avoid complex direct IK/motion planning
        # from the pick hover configuration.
        ...
        if obj is not None:
            from rlbench.backend.utils import compute_tip_attachment, update_attached_pose
        ...
        if traj_to_home:
            for conf in traj_to_home:
                env.set_robot_conf(conf)
                self._step_sim()
                if obj is not None:
                    if mug_tip_offset_local is not None and mug_tip_quat_local is not None and update_attached_pose is not None:
                        update_attached_pose(gripper_tip, obj, mug_tip_offset_local, mug_tip_quat_local)
                    elif mug_tip_offset_local is not None:
        ...
        q_start = tuple(env.get_robot_conf())  # Will be home_q
        init = [
            ('conf', q_start),
            ('at-conf', q_start),
            ('holding', object_name),
