...
def main():
    ...
    try:
        q_pick_hover = env.compute_hover_config(mug, list(pose), hover_offset=0.125)
    ...
    try:
        # Find best placement in groceries boundary
        place_pose = env.find_best_placement(mug, 'groceries_boundary')
    ...
    # --- PDDL PLANNING START ---
    print("Setting up PDDL problem for 'hover-pick-hover' sequence...")
    ...
    # Goal: Holding the mug AND being at the place-hover configuration
    goal = And(('holding', 'mug_box'), ('at-conf', q_place_hover_tuple))
    ...
    print("Hover-pick-hover complete. Press Ctrl+C to close.")
    try:
        while True:
            pr.step()
